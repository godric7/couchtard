...
function async_call(items, funcname, callback) {
  var errors = Array.isArray(items) ? [] : {};
  var length = Object.keys(items).length;

  var step = function(idx, err) {
    length -=1;
    if (err && true)
      errors[idx] = err;
  ...
  for (var idx in items) {
    var done = step.bind(null, idx);
    items[idx][funcname](done);
  }
}
...
Source.prototype.add_multi = function(items, opts, callback) {
  ...
  if (typeof opts == 'function') { callback = opts; opts = {}; }
  async_call(items, 'before_save', function(err, errs) {
    if (err) { return callback(err); }
    async_call(items, 'validate', function(err, errs) {
      if (err) { return callback(err); }
...
Source.prototype.delete_multi = function(items, opts, callback) {
  ...
  if (typeof opts == 'function') { callback = opts; opts = {}; }
  async_call(items, 'before_delete', function(err, errs) {
    if (err) { return callback(err); }
