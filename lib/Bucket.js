...
function async_each(items, func, callback) {
  var flag = false;
  var errors = {};
  var results = {};
  var length = Object.keys(items).length;

  var step = function(key, error, result) {
    length--;
    if (error) 
      errors[key] = error;
    if (result) 
...
Bucket.prototype._add = function(items, opts, callback) {
  ...
  async_each(items, function(item, idx, done) {
...
Bucket.prototype._lock = function(keys, opts, callback) {
  ...
  async_each(keys, function(key, idx, done) {
